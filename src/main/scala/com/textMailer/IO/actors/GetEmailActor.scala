...
object GetEmailActor {
  case class GetGmailMessages(messageIds: List[String], gmailUserId: String, accessToken: String, emailAddress: String)
}
...
class GetEmailActor extends Actor {
  ...
  def receive = {
    case GetGmailMessages(messageIds, gmailUserId, accessToken, emailAddress) => {
      ...
      val batchRes = batchReq.apply.map(r => {
        val messageBodies = (r.toJValue.values.asInstanceOf[Map[String,String]].get("body") match {
          case Some(b) => findJsonObjects(b).map(json => saveEmailActor ! SaveGmailMessage(json, emailAddress))
          case None => List()
        })
      })
    }
